//------------------------------------------------------------------------------------------------
class GME_MultilineEditBoxComponent : SCR_ChangeableComponentBase 
{
	[Attribute("0.1")]
	...
	void OnValueChanged()
	{
		...
		AnimateWidget.Opacity(m_wWarningIcon, 0, m_fColorsAnimationTime);
	...
	void OnHandlerFocusLost()
	{
		...
		AnimateWidget.Opacity(m_wBackground, m_fOpacityDefault, m_fAnimationRate);
		m_bIsTyping = false;
	...
	void OnInvalidInput()
	{
		AnimateWidget.Opacity(m_wWarningIcon, 1, m_fColorsAnimationTime);
