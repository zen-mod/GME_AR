...
{
	static int GME_emptyTerrainAreaRadius = 20;
	
	/*!
	Functions to tell the server to move an entity
	*/
	void GME_TeleportEntityToPlayer(RplId sourceEntityId, RplId targetEntityId)
	{
		Rpc(GME_Server_TeleportEntityToPlayer, sourceEntityId, targetEntityId);
	}
	
	[RplRpc(RplChannel.Reliable, RplRcver.Server)]
	protected void GME_Server_TeleportEntityToPlayer(RplId entityId, RplId targetEntityId)
	{		
		// get entity from id 
		...
		vector target_pos;
		SCR_WorldTools.FindEmptyTerrainPosition(target_pos, targetEntity.GetOrigin(), GME_emptyTerrainAreaRadius);
